...
// ===== A PARTIR DAQUI SÓ CORRE EM GET (renderização da página) =====
...
@-- HTML e JavaScript do admin-perfil.php --
...
.secao-conteudo:hover { box-shadow: var(--shadow-lg); transform: translateY(-2px); }
...
.secao-titulo i { color: var(--primary); font-size: 1.5rem; }
.secao-titulo::after {
    content: '';
    position: absolute;
    bottom: -2px; left: 0;
...
.perfil-foto { display: flex; flex-direction: column; align-items: center; gap: 12px; }
...
.foto-acoes { display: flex; gap: 8px; margin-top: 5px; }
...
.btn-icone:hover { background: #0a9396; border-color: #0a9396; color: white; transform: translateY(-2px); }
.btn-remover-foto:hover { background: #dc3545; border-color: #dc3545; }

.perfil-info { flex: 1; }
.perfil-info h2 {
    font-size: 28px; font-weight: 700; color: #003072;
    margin: 0 0 8px 0;
    display: flex; align-items: center; gap: 12px; flex-wrap: wrap;
}
.badge-ativo { display: inline-block; padding: 4px 12px; background: #e8f5e9; color: #2e7d32; border-radius: 20px; font-size: 12px; font-weight: 600; }
.badge-inativo { background: #fee2e2; color: #c62828; }

.perfil-info .email { color: #6c757d; margin-bottom: 12px; font-size: 15px; display: flex; align-items: center; gap: 8px; }
.perfil-info .email i { color: #0a9396; width: 20px; }
.perfil-info .nivel {
    display: inline-flex; align-items: center; gap: 8px;
    padding: 6px 16px;
    background: linear-gradient(135deg, #0a9396, #003072);
    color: white; border-radius: 30px; font-size: 13px; font-weight: 600;
    box-shadow: 0 2px 8px rgba(10,147,150,0.3);
}
.perfil-info .nivel i { font-size: 14px; }

.perfil-stats { display: flex; gap: 30px; margin-top: 20px; padding-top: 20px; border-top: 1px solid #eef2f6; }
.perfil-stat { text-align: center; }
.perfil-stat .stat-number { font-size: 22px; font-weight: 700; color: #003072; display: block; }
.perfil-stat .stat-label  { font-size: 12px; color: #6c757d; }

@media (max-width: 768px) {
    .perfil-header { flex-direction: column; text-align: center; padding: 25px; }
    .perfil-info h2  { justify-content: center; }
    .perfil-info .email { justify-content: center; }
    .perfil-stats { justify-content: center; }
}

.linha-form { display: grid; grid-template-columns: repeat(2, 1fr); gap: 25px; margin-bottom: 25px; }
.grupo-form { display: flex; flex-direction: column; gap: 8px; }
.grupo-form label { font-size: 0.85rem; font-weight: 700; color: var(--dark); text-transform: uppercase; letter-spacing: 0.5px; }
.grupo-form label i { margin-right: 8px; color: var(--primary); }

.campo-form { padding: 12px 16px; border: 2px solid var(--light-gray); border-radius: 12px; font-size: 0.95rem; transition: var(--transition); background: var(--white); }
.campo-form:focus { outline: none; border-color: var(--primary); box-shadow: 0 0 0 3px rgba(10,147,150,0.1); }
.campo-form:hover:not(:focus) { border-color: var(--primary-light); }

.area-texto { resize: vertical; min-height: 100px; font-family: inherit; }
.contador-caracteres { font-size: 0.75rem; color: var(--gray); text-align: right; margin-top: 6px; }

.info-seguranca { background: linear-gradient(135deg, #f8f9fa, #ffffff); border-radius: 16px; padding: 20px; margin-bottom: 25px; border: 1px solid var(--light-gray); }
.info-item { display: flex; justify-content: space-between; align-items: center; padding: 12px 0; border-bottom: 1px solid var(--light-gray); transition: var(--transition); }
.info-item:last-child { border-bottom: none; }
.info-item:hover { transform: translateX(5px); }
.info-label { font-weight: 700; color: var(--dark); display: flex; align-items: center; gap: 10px; }
.info-label i { color: var(--primary); width: 20px; }
.info-value { font-weight: 500; padding: 4px 12px; border-radius: 20px; }
.info-value.ativo  { background: #d4edda; color: var(--success); }
.info-value.inativo{ background: #f8d7da; color: var(--danger); }

.btn-primario {
    padding: 12px 28px;
    background: linear-gradient(135deg, var(--primary), var(--primary-dark));
    color: var(--white); border: none; border-radius: 50px; font-weight: 600;
    cursor: pointer; transition: var(--transition);
    display: inline-flex; align-items: center; gap: 10px; font-size: 0.9rem;
}
.btn-primario:hover { transform: translateY(-2px); box-shadow: var(--shadow-lg); background: linear-gradient(135deg, var(--primary-dark), var(--primary)); }

.btn-cancelar, .btn-salvar {
    padding: 12px 32px; border-radius: 50px; font-weight: 600; cursor: pointer;
    border: none; transition: var(--transition); font-size: 0.9rem;
    display: inline-flex; align-items: center; gap: 10px;
}
.btn-cancelar { background: var(--light-gray); color: var(--gray); }
.btn-cancelar:hover { background: #dee2e6; transform: translateY(-2px); }
.btn-salvar { background: linear-gradient(135deg, var(--primary), var(--primary-dark)); color: var(--white); }
.btn-salvar:hover { transform: translateY(-2px); box-shadow: var(--shadow-lg); }

.notificacoes-grid { display: flex; flex-direction: column; gap: 15px; }
.toggle-item { display: flex; justify-content: space-between; align-items: center; padding: 18px; background: linear-gradient(135deg, #f8f9fa, #ffffff); border-radius: 16px; cursor: pointer; transition: var(--transition); border: 1px solid var(--light-gray); }
.toggle-item:hover { transform: translateX(8px); box-shadow: var(--shadow-sm); border-color: var(--primary-light); }
.toggle-info { display: flex; align-items: center; gap: 18px; }
.toggle-info i { font-size: 24px; color: var(--primary); transition: var(--transition); }
.toggle-item:hover .toggle-info i { transform: scale(1.1); }
.toggle-info strong { display: block; font-size: 0.95rem; color: var(--dark); margin-bottom: 4px; }
.toggle-info span { font-size: 0.8rem; color: var(--gray); }
.toggle-switch { position: relative; width: 52px; height: 28px; display: inline-block; }
.toggle-switch input { opacity: 0; width: 0; height: 0; }
.toggle-slider { position: absolute; cursor: pointer; top: 0; left: 0; right: 0; bottom: 0; background-color: var(--gray); transition: 0.3s; border-radius: 34px; }
.toggle-slider:before { position: absolute; content: ""; height: 22px; width: 22px; left: 3px; bottom: 3px; background-color: var(--white); transition: 0.3s; border-radius: 50%; box-shadow: var(--shadow-sm); }
input:checked + .toggle-slider { background: linear-gradient(135deg, var(--primary), var(--primary-dark)); }
input:checked + .toggle-slider:before { transform: translateX(24px); }

.stats-pessoais { display: grid; grid-template-columns: repeat(3, 1fr); gap: 25px; }
.stat-card { display: flex; align-items: center; gap: 20px; padding: 25px; background: linear-gradient(135deg, var(--white), #f8f9fa); border-radius: 16px; transition: var(--transition); border: 1px solid var(--light-gray); position: relative; overflow: hidden; }
...
.stat-card i { font-size: 40px; color: var(--primary); transition: var(--transition); }
.stat-card:hover i { transform: scale(1.1); }
.stat-card h3 { font-size: 28px; font-weight: 800; margin: 0; color: var(--secondary); line-height: 1; }
.stat-card p { font-size: 0.85rem; color: var(--gray); margin: 5px 0 0 0; }

.tabela-acessos { overflow-x: auto; border-radius: 16px; }
.tabela-dados { width: 100%; border-collapse: collapse; }
.tabela-dados th { text-align: left; padding: 15px; background: linear-gradient(135deg, #f8f9fa, #e9ecef); font-weight: 700; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.5px; color: var(--dark); }
.tabela-dados td { padding: 12px 15px; border-bottom: 1px solid var(--light-gray); font-size: 0.9rem; transition: var(--transition); }
.tabela-dados tr:hover td { background: #f8f9fa; transform: scale(1.01); }

.empty-state { text-align: center; padding: 60px; color: var(--gray); }
.empty-state i { font-size: 64px; margin-bottom: 20px; display: block; color: var(--primary-light); }
.empty-state p { font-size: 1rem; }

.secao-acoes { display: flex; justify-content: flex-end; gap: 20px; margin-top: 20px; padding: 20px; background: #f8f9fa; border-radius: 16px; }

.modal { display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.8); backdrop-filter: blur(4px); z-index: 10000; align-items: center; justify-content: center; padding: 20px; animation: fadeInUp 0.3s ease-out; }
.modal.ativo { display: flex; }
.modal-conteudo { background: var(--white); border-radius: 24px; max-width: 600px; width: 100%; max-height: 90vh; overflow-y: auto; box-shadow: var(--shadow-xl); animation: slideIn 0.3s ease-out; }
.modal-cabecalho { display: flex; justify-content: space-between; align-items: center; padding: 25px 30px; border-bottom: 2px solid var(--light-gray); background: linear-gradient(135deg, #f8f9fa, var(--white)); }
.modal-cabecalho h2 { font-size: 1.5rem; font-weight: 700; color: var(--secondary); display: flex; align-items: center; gap: 10px; }
.modal-cabecalho h2 i { color: var(--primary); }
.modal-fechar { width: 40px; height: 40px; border-radius: 50%; background: var(--white); border: 1px solid var(--light-gray); cursor: pointer; transition: var(--transition); display: flex; align-items: center; justify-content: center; }
.modal-fechar:hover { background: var(--danger); color: var(--white); transform: rotate(90deg); }
.modal-corpo { padding: 30px; }
.modal-acoes { display: flex; justify-content: flex-end; gap: 15px; margin-top: 30px; padding-top: 20px; border-top: 1px solid var(--light-gray); }

.forca-senha { margin-top: 10px; }
.barra-forca { height: 6px; background: var(--light-gray); border-radius: 3px; overflow: hidden; }
.preenchimento-forca { height: 100%; width: 0; transition: width 0.3s ease; }
.rotulo-forca { font-size: 0.75rem; margin-top: 8px; font-weight: 600; }

.requisitos-senha { background: linear-gradient(135deg, #f8f9fa, var(--white)); border-radius: 16px; padding: 20px; margin: 20px 0; border: 1px solid var(--light-gray); }
.requisitos-senha h4 { font-size: 0.85rem; margin-bottom: 12px; color: var(--dark); font-weight: 700; }
.requisito { font-size: 0.8rem; margin-bottom: 8px; display: flex; align-items: center; gap: 10px; color: var(--gray); transition: var(--transition); }
.requisito.atendido { color: var(--success); }
.requisito i { font-size: 0.75rem; }

.checkbox-label { display: flex; align-items: center; gap: 10px; cursor: pointer; font-weight: normal; padding: 8px; border-radius: 8px; transition: var(--transition); }
...
.notificacao i { font-size: 1.2rem; }

@media (max-width: 1024px) { .secao-conteudo { padding: 25px; } .stats-pessoais { gap: 20px; } }
@media (max-width: 768px) {
    .linha-form { grid-template-columns: 1fr; gap: 20px; }
    .stats-pessoais { grid-template-columns: 1fr; gap: 15px; }
    .perfil-header { flex-direction: column; text-align: center; padding: 20px; }
    .secao-acoes { flex-direction: column; }
    .btn-cancelar, .btn-salvar { width: 100%; justify-content: center; }
    .modal-conteudo { margin: 20px; max-width: calc(100% - 40px); }
    .toggle-item { flex-direction: column; align-items: flex-start; gap: 15px; }
    .toggle-info { width: 100%; }
    .info-item { flex-direction: column; align-items: flex-start; gap: 8px; }
}
@media (max-width: 480px) {
    .secao-conteudo { padding: 20px; }
    .perfil-info h2 { font-size: 24px; }
    .foto-container { width: 100px; height: 100px; }
    .stat-card { padding: 20px; }
    .stat-card i { font-size: 32px; }
    .stat-card h3 { font-size: 24px; }
    .modal-cabecalho { padding: 20px; }
...
.carregando { position: relative; pointer-events: none; opacity: 0.7; }
.carregando::after { content: ''; position: absolute; top: 50%; left: 50%; width: 30px; height: 30px; margin: -15px 0 0 -15px; border: 3px solid var(--light-gray); border-top-color: var(--primary); border-radius: 50%; animation: spin 0.6s linear infinite; }
...
document.getElementById('uploadFoto')?.addEventListener('change', async function(e) {
    ...
    if (file.size > 5 * 1024 * 1024) {
        mostrarNotificacao('Máximo 5MB', 'erro');
        this.value = '';
        return;
    }
...
function verificarForcaSenha() {
    ...
    const forca     = [comprimentoOk, maiusculaOk, minusculaOk, numeroOk, especialOk].filter(Boolean).length;
...
function mostrarNotificacao(mensagem, tipo) {
    ...
    notif.style.background = tipo === 'sucesso' ? '#28a745' : '#dc3545';
